...
class MiscProcessorMixin:
    ...
    def apply_align_leading_digits(self: GalTextT) -> GalTextT:
        """
        根据 raw_message 自动对齐开头的数字。
        如果原始文本以数字开头，而当前 message 没有，则将原始开头的数字部分提取并补回。
        用于处理那些带编号的脚本行。
        """
        for item in self.items:
            raw_msg = item.get("raw_message")
            ...
            if isinstance(raw_msg, str) and isinstance(msg, str):
                # 匹配 raw_message 开头的连续数字
                match = re.match(r"^([A-Za-z]*\d+)", raw_msg)
                if match:
                    leading_digits = match.group()
                    if not msg.startswith(leading_digits):
                        item["message"] = f"{leading_digits}{msg}"
        return self
    ...
    def apply_extract_control_tags(self: GalTextT) -> GalTextT:
        """
        全能剥离：处理 V编号、布局码($br/$c) 和 无语义占位符($o)。
        保留语义变量（如 $s0, $s1）在 message 中，确保翻译逻辑正确。
        """
        # 1. 前缀正则：
        # (V[A-Z]*\d+) -> 语音编号
        # (\$o) -> 无语义占位符
        # (\$(?:br|c\d+)[a-zA-Z0-9=._-]*) -> 布局/颜色控制码
        prefix_pattern = re.compile(
            r"^(\u3000*)((?:(?:V[A-Z]*\d+)|\$o|(?:\$(?:br|c\d+)[a-zA-Z0-9=._-]*))+)"
        )

        # 2. 后缀正则：
        suffix_pattern = re.compile(
            r"((?:\$o|(?:\$(?:br|c\d+|C\d+)[a-zA-Z0-9=._-]*))+)$"
        )
        ...
        for item in self.items:
            ...
            # 剥离前缀
            prefix_match = prefix_pattern.search(message)
            if prefix_match:
                spaces = prefix_match.group(1)
                tech_prefix = prefix_match.group(2)

                item["prefix"] = tech_prefix
                # 重新组合：保留空格，扣掉技术前缀
                message = spaces + message[len(spaces) + len(tech_prefix) :]

            # 剥离后缀
            suffix_match = suffix_pattern.search(message)
            if suffix_match:
                item["suffix"] = suffix_match.group(1)
                message = message[: -len(item["suffix"])]

            item["message"] = message
        return self

    def apply_reassemble_control_tags(self: GalTextT) -> GalTextT:
        """
        重新组装：技术前缀强制置于最左侧，随后是空格和含人名变量的正文。
        """
        for item in self.items:
            message = item.get("message")
            if not isinstance(message, str):
            ...
            # 使用 pop 确保还原后 item 中不再残留 prefix/suffix 字段
            ...
            if prefix:
                ...
                content = message[len(spaces) :]
                # 还原：[控制码][空格][正文]
                message = f"{prefix}{spaces}{content}"
